<template>
    <div class="content">
        <h2>TOOL BUKA VALIDASI PERSIL</h2>
        <div class="section">
            <div class="form-loc">
...
export default defineComponent({
    ...
    methods: {
        ...
        start() {
            ...
            const data = JSON.parse(text) as FileInterface[];

            window.COMM.botStartBukaValidasiPersil(this.user, this.pass, this.kabupatenId, this.kecamatanId, this.desaId, data, this.fileLocBtnTxt);
        },
        updateStatusValidasi(event: Electron.IpcRenderer, data: any[]) {
            (this.reportJson as String[]).push(`${data[0].nib},${data[0].msg},${data[0].status}`);
        ...
        async updateFileSelect(event: Electron.IpcRenderer, data: any[]) {
            this.data = data[1];
            this.cData = data[1].length;

            this.fileLocBtnTxt = data[0];
        },
    },
    mounted() {
        document.title = "AutoMate - Buka Validasi Persil";
        window.COMM.fileSelected(this.updateFileSelect);
        window.COMM.botStatusHandler(this.updateStatusValidasi);
        this.reportJson.push('pid,message,isberhasil');
