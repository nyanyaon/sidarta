...
export class ValidasiPersilBot extends Bot {
    ...
    async startPid(user: string, pass: string, kabupatenId: string,kecamatanId: string, desaId: string, fileLoc: string) {
        try {
            const rStream = fs.createReadStream(fileLoc, {encoding: "utf-8"});
            const arrStatusValidasi: StatusValidasi[] = [];
            let totalnib: number;
            ...
            page.on('response', async (response) => {
                ...
                if (request.url().includes('QueryByNIB')) {
                    ...
                    await page.evaluate(async (pid) => {
                        await fetch("https://peta.atrbpn.go.id/DataSpasial/ValidasiBidang", {
                            "headers": {
                                "accept": "*/*",
                                "accept-language": "en-US,en;q=0.9",
                                "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
            ...
            rStream.on("data", async (chunk) => {
                ...
                while ((nib = str.pop()) !== undefined) {
                    if(nib == "") continue;
                    console.log(nib);
                    const status = await page.evaluateHandle(async (kabupatenId, kecamatanId, desaId, nib) => {
                        await fetch("https://peta.atrbpn.go.id/DataSpasial/QueryByNIB", {
                            "headers": {
                                "accept": "application/json, text/javascript, */*; q=0.01",
                                "accept-language": "en-US,en;q=0.9",
                                "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
